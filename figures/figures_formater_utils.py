...
Generic functions for figure data formaters.
    ...
    if smooth_factor > 1:
        ...
        grid = zoom(input=grid, zoom=smooth_factor, order=1)
        latitudes = numpy.linspace(latitudes[0], latitudes[-1], grid.shape[0])
        longitudes = numpy.linspace(longitudes[0], longitudes[-1], grid.shape[1])
...
REFERENCE_MODEL_PARAMETERS: dict[str, str | bool | float] = {
    ...
    "elastic mean\nbarystatic sea\nlevel": "mean",
    "GRACE\nsolution": "DDK7/TREND_GRACE(-FO)_MSSA_2003_2022_NoGIA_PELTIER_ICE6G-D.csv",
}
...
def preprocess_grid(
    load_model: ElasticLoadModel, saturation_threshold: float = 50.0
    ...
    # Applies the saturation threshold.
    if saturation_threshold is not None:

        grid = numpy.clip(a=grid, a_min=-saturation_threshold, a_max=saturation_threshold)
        ...
        for option in ANELASTICITY_OPTIONS:
            ...
            for parameter in sub_parameters:
                ...
                for combination in numpy.unique(
                    ...
                    results[metric][option][parameter] += [numpy.std(sub_sub_dataframe[metric])]
