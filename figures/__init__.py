"""
Contains figrue functions, figure data functions and figure data files.
"""

from .figures_data_formater import (
    preprocess_figure_1,
    preprocess_figure_2,
    preprocess_figure_3,
    preprocess_figure_4,
)
from .figures_generation import (
    generate_figure_1,
    generate_figure_2,
    generate_figure_3,
    generate_figure_4,
)
from .figures_sup_data_formater import (
    preprocess_figure_sup_1,
    preprocess_figure_sup_2,
    preprocess_figure_sup_3,
    preprocess_figure_sup_4,
    preprocess_figure_sup_5,
    preprocess_figure_sup_6,
    preprocess_figure_sup_7,
    preprocess_figure_sup_8,
    preprocess_figure_sup_9,
    preprocess_figure_sup_10,
)
from .figures_sup_generation import (
    generate_figure_sup_1,
    generate_figure_sup_2,
    generate_figure_sup_3,
    generate_figure_sup_4,
    generate_figure_sup_5,
    generate_figure_sup_6,
    generate_figure_sup_7,
    generate_figure_sup_8,
    generate_figure_sup_9,
    generate_figure_sup_10,
)

objects = [
    preprocess_figure_1,
    preprocess_figure_2,
    preprocess_figure_3,
    preprocess_figure_4,
    generate_figure_1,
    generate_figure_2,
    generate_figure_3,
    generate_figure_4,
    preprocess_figure_sup_1,
    preprocess_figure_sup_2,
    preprocess_figure_sup_3,
    preprocess_figure_sup_4,
    preprocess_figure_sup_5,
    preprocess_figure_sup_6,
    preprocess_figure_sup_7,
    preprocess_figure_sup_8,
    preprocess_figure_sup_9,
    preprocess_figure_sup_10,
    generate_figure_sup_1,
    generate_figure_sup_2,
    generate_figure_sup_3,
    generate_figure_sup_4,
    generate_figure_sup_5,
    generate_figure_sup_6,
    generate_figure_sup_7,
    generate_figure_sup_8,
    generate_figure_sup_9,
    generate_figure_sup_10,
]
